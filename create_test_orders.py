...
def create_test_orders():
    ...
    pesanan1 = Pesanan.objects.create(
        meja=meja_1,
        status='BARU',
        catatan='Ayam gorengnya extra crispy ya',
        created_at=timezone.now() - timedelta(minutes=5)
    )
    
    DetailPesanan.objects.create(
        pesanan=pesanan1,
        menu_item=nasi_gudeg,        jumlah=2,
        harga=nasi_gudeg.harga,
    ...
    DetailPesanan.objects.create(
        ...
        jumlah=1,
        harga=ayam_bakar.harga,
        catatan_item='Extra crispy'
    )
    ...
    # Hitung total
    pesanan1.total_harga = sum(detail.jumlah * detail.harga for detail in pesanan1.detailpesanan_set.all())
    pesanan1.save()
    ...
    DetailPesanan.objects.create(
        ...
        menu_item=es_jeruk,
        jumlah=1,
        harga=es_jeruk.harga
    )
    
    # Hitung total
    pesanan2.total_harga = sum(detail.jumlah * detail.harga for detail in pesanan2.detailpesanan_set.all())
    pesanan2.save()
    ...
    pesanan3 = Pesanan.objects.create(
        meja=meja_3,
        status='SELESAI',        created_at=timezone.now() - timedelta(minutes=25)
    )
    ...
    # Hitung total
    pesanan3.total_harga = sum(detail.jumlah * detail.harga for detail in pesanan3.detailpesanan_set.all())
    pesanan3.save()
